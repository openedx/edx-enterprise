...
import enterprise
from common.djangoapps.third_party_auth.models import SAMLConfiguration, SAMLProviderData
...
# which is injected with the same SSO_PASSWORD env var at realm import time).
SSO_PASSWORD = os.environ['SSO_PASSWORD']
...
# ---------------------------------------------------------------------------
# Step 2: Seed the globally-scoped enterprise operator user (once, not per tenant)
# ---------------------------------------------------------------------------
# The operator applies across all enterprises, so it is created a single time
# here and never linked to a specific enterprise.
print('\n--- Step 2: Seed global enterprise operator user ---')
ensure_enterprise_groups()
seed_global_operator_user()
...
for tenant in TENANTS:
    ...
    # login account is created in Step D.  The enterprise slug is
    ...
    # Step B: Create the tenant's SAML IdP (SAMLProviderConfig) and link it to the
    # enterprise so tpa_hint and SSO logins resolve enterprise context.
    print('--- Step B: Create SAML IdP and link to enterprise ---')
    create_enterprise_saml_provider(
    ...
    print(f'Created SAML IdP saml-{name} (entity_id={idp_entity_id}) linked to {ec.name}')

    # Step C: Set enterprise branding (logo + house colors) so the logistration
    # sidebar renders a visually distinct, verifiable brand per tenant.
    print('--- Step C: Set enterprise branding ---')
    update_or_create_enterprise_branding(
        enterprise_customer=ec,
    ...
    # Step D: Create the fully-linked LMS user.
    # "Fully-linked" means this SSO user in Keycloak is linked to an existing
    # LMS user, AND the LMS user is linked to an existing enterprise customer.
    print('--- Step D: Create fully-linked LMS user ---')
    ...
    # Step E: Enforce that the newcomer SSO user has NO LMS user.
    # Registering via SSO in a prior run creates one, so delete it every run to
    # idempotently converge back to a "no account" state.
    print('--- Step E: Ensure newcomer has no LMS user ---')
...
# ---------------------------------------------------------------------------
# Update the LMS with SAML metadata from Keycloak.
#
# Keycloak in devstack is served over plain HTTP (edx.devstack.keycloak:8080),
# but openedx-platform enforces HTTPS on SAML metadata URLs as an SSRF defense
# (validate_saml_metadata_url, added upstream in 70a56246 / GHSA-328g-7h4g-r2m9).
# Bypass that guard for this local-only pull only; scoping the patch to this call
# keeps the security check active everywhere else. The validator is imported by
# name into the tasks module, so patch it there.
