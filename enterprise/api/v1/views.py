...
class EnterpriseViewSet:
    ...
    def ensure_data_exists(self, request, data, error_message=None):
        ...
        if not data:
            error_message = (
                error_message
                or "Unable to fetch API response from endpoint '{}'.".format(
...
class EnterpriseReadOnlyModelViewSet(
    EnterpriseModelViewSet, viewsets.ReadOnlyModelViewSet
):
    """
...
class EnterpriseCustomerViewSet(EnterpriseReadWriteModelViewSet):
    ...
    @permission_required(
        "enterprise.can_access_admin_dashboard", fn=lambda request, pk: pk
    )
    def partial_update(self, request, *args, **kwargs):
        return super().partial_update(request, *args, **kwargs)

    @method_decorator(
        require_at_least_one_query_parameter("course_run_ids", "program_uuids")
    )
    @action(detail=True)
    @permission_required(
    ...
    def contains_content_items(self, request, pk, course_run_ids, program_uuids):
        ...
        course_run_ids = [
            unquote(quote_plus(course_run_id)) for course_run_id in course_run_ids
        ]
    ...
    @action(
        methods=["post"], permission_classes=[permissions.IsAuthenticated], detail=True
    )
    ...
    def enroll_learners_in_courses(self, request, pk):
        ...
        enterprise_customer = self.get_object()
        serializer = (
            serializers.EnterpriseCustomerBulkSubscriptionEnrollmentsSerializer(
                data=request.data,
                context={
                    "enterprise_customer": enterprise_customer,
                    "request_user": request.user,
        ...
        try:
            ...
            error_message = (
                "Something went wrong while validating bulk enrollment requests."
        ...
        enrollments_info = serialized_data.get(
            "licenses_info", serialized_data.get("enrollments_info")
        )
        ...
        course_runs_modes = {
            enrollment_info["course_run_key"]: None
            for enrollment_info in enrollments_info
        }
        ...
        for email in emails:
            try:
                validate_email_to_link(
                    email, enterprise_customer, raise_exception=False
                )
        ...
        enrollments_info = [
            info
            for info in enrollments_info
            if info.get("email") not in email_errors
            and info.get("user_id") not in user_id_errors
        ]

        results = enroll_subsidy_users_in_courses(
            enterprise_customer, enrollments_info, discount
        )
        ...
        for course_run in course_runs_modes:
            ...
            if len(pending_users | existing_users) > 0:
                LOGGER.info(
                    "Successfully bulk enrolled learners: {} into course {}".format(
                        pending_users | existing_users,
                        course_run,
                    )
                )
                track_enrollment(
                    PATHWAY_CUSTOMER_ADMIN_ENROLLMENT, request.user.id, course_run
                )
    ...
    @action(
        methods=["patch"], detail=True, permission_classes=[permissions.IsAuthenticated]
    )
    @permission_required("enterprise.can_access_admin_dashboard")
        ...
        with transaction.atomic():
            for email in emails_to_unlink:
                try:
                    ...
                    models.PendingEnterpriseCustomerUser.DoesNotExist,
                ):
                    msg = "User with email {} does not exist in enterprise {}.".format(
                        email, enterprise_customer
                    )
                    LOGGER.warning(msg)
                except Exception as exc:
                    msg = "Could not unlink {} from {}".format(
                        email, enterprise_customer
                    )
...
class EnterpriseSubsidyFulfillmentViewSet(EnterpriseWrapperApiViewSet):
    ...
    def get_queryset(self):
        ...
        """
        enterprise_customer_uuid = get_enterprise_customer_from_user_id(
        ...
        learner_credit_enrollments = (
            models.LearnerCreditEnterpriseCourseEnrollment.objects.filter(
                uuid=fulfillment_source_uuid
            )
        )
        if not self.request.user.is_staff:
            learner_credit_enrollments = learner_credit_enrollments.filter(
                subsidy_fulfillment_filter
            )
        ...
        if not self.request.user.is_staff:
            licensed_enrollments = licensed_enrollments.filter(
                subsidy_fulfillment_filter
            )
        ...
        raise ValidationError(
            "No enrollment found for the given fulfillment source uuid.",
    ...
    @permission_required(
        ...
        fn=lambda request, fulfillment_source_uuid: get_enterprise_customer_from_user_id(
            request.user.id
        ),
    )
    def retrieve(self, request, fulfillment_source_uuid, *args, **kwargs):
        ...
        try:
            ...
            return Response(status=HTTP_404_NOT_FOUND, data={"detail": exc.detail})
    ...
    def cancel_enrollment(self, request, fulfillment_source_uuid):
        ...
        try:
            username = (
                enrollment.enterprise_course_enrollment.enterprise_customer_user.username
            )
...
class LicensedEnterpriseCourseEnrollmentViewSet(EnterpriseWrapperApiViewSet):
    ...
    def _terminate_enrollment(self, enterprise_enrollment, course_overview):
        ...
        if self._has_user_completed_course_run(enterprise_enrollment, course_overview):
            LOGGER.info(
                ...
                "in Enterprise {enterprise}, course is already complete.".format(
                    **log_message_kwargs
                )
            )
        ...
        if CourseMode.mode_for_course(course_run_id, audit_mode):
            ...
            try:
                ...
                LOGGER.info(
                    ...
                    "from Course {course_id} that contains no audit mode.".format(
                        **log_message_kwargs
                    )
                )
    ...
    def _course_enrollment_modified_at_by_user_and_course_id(
        self, licensed_enrollments
    ):
        """
        ...
        enterprise_course_enrollments = [
            licensed_enrollment.enterprise_course_enrollment
            for licensed_enrollment in licensed_enrollments
        ]
        user_ids = [
            str(ece.enterprise_customer_user.user_id)
        ...
        course_enrollment_histories = CourseEnrollment.history.filter(
            user_id__in=user_ids, course_id__in=course_ids
    ...
    @permission_required(
        "enterprise.can_access_admin_dashboard",
        fn=lambda request: request.data.get("enterprise_id"),
    )
    def license_revoke(self, request, *args, **kwargs):
        ...
        if not all(dependencies):
            raise NotConnectedToOpenEdX(
                _(
                    "To use this endpoint, this package must be "
        ...
        enrollments_by_course_id = self._enrollments_by_course_for_licensed_user(
            enterprise_customer_user
        )

        revocation_results = {}
        any_failures = False
        for course_overview in get_course_overviews(
            ...
            try:
                ...
                revocation_results[course_id] = {
                    "success": True,
                    "message": revocation_status,
                }
                if (
                    revocation_status
                    != self.EnrollmentTerminationStatus.COURSE_COMPLETED
                ):
    ...
    def bulk_licensed_enrollments_expiration(self, request):
        ...
        ignore_enrollments_modified_after = get_request_value(
            request, self.OPT_IGNORE_ENROLLMENTS_MODIFIED_AFTER_PARAM, None
        )

        if not expired_license_uuids:
            return Response(
                "Parameter {} must be provided".format(
        ...
        if ignore_enrollments_modified_after:
            ignore_enrollments_modified_after = parse_datetime(
        ...
        indexed_overviews = {
            overview.get("id"): overview for overview in course_overviews
        }
        ...
        for licensed_enrollment in licensed_enrollments:
            ...
            if ignore_enrollments_modified_after:
                key = f"{user_id}{course_id}"
                course_enrollment_modified_at = (
                    course_enrollment_modified_at_by_user_and_course_id[key]
                )
                if course_enrollment_modified_at >= ignore_enrollments_modified_after:
                    LOGGER.info(
                        "Enrollment termination: not updating enrollment in {} for User {} "
                        "course enrollment has been modified past {}.".format(
                            course_id, user_id, ignore_enrollments_modified_after
                        )
                    )
                    continue
            ...
            try:
                termination_status = self._terminate_enrollment(
                    enterprise_course_enrollment, course_overview
                )
...
class PendingEnterpriseCustomerUserViewSet(EnterpriseReadWriteModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return_status = self._get_return_status(
            serializer, many=isinstance(request.data, list)
        )
        headers = self.get_success_headers(serializer.data)
        return Response(serializer.data, status=return_status, headers=headers)


class PendingEnterpriseCustomerUserEnterpriseAdminViewSet(
    ...
    def link_learners(self, request, enterprise_uuid):
        ...
        if not request.data:
            LOGGER.error(
                "Empty user email payload in link_learners for enterprise: %s",
                enterprise_uuid,
            )
            return Response(
...
class EnterpriseCustomerBrandingConfigurationViewSet(EnterpriseReadWriteModelViewSet):
    ...
    def update_branding(self, request, enterprise_uuid):
        ...
        try:
            ...
            branding_configs = (
                models.EnterpriseCustomerBrandingConfiguration.objects.filter(
                    enterprise_customer=enterprise_customer
                )
            )
            if len(branding_configs) > 0:
                branding_config = (
                    models.EnterpriseCustomerBrandingConfiguration.objects.get(
                        enterprise_customer=enterprise_customer
                    )
                )
            else:
            ...
            LOGGER.exception("Error with updating branding configuration")
            return Response(
...
class EnterpriseCustomerCatalogViewSet(EnterpriseReadOnlyModelViewSet):
    ...
    @permission_required("enterprise.")
    @permission_required(
        "enterprise.can_view_catalog", fn=lambda request, *args, **kwargs: None
    )
    ...
    @permission_required(
        "enterprise.can_view_catalog",
        fn=lambda request, *args, **kwargs: get_enterprise_customer_from_catalog_id(
            kwargs["pk"]
        ),
    )
    ...
    def contains_content_items(self, request, pk, course_run_ids, program_uuids):
        ...
        if program_uuids:
            contains_content_items = (
                contains_content_items
                and enterprise_customer_catalog.contains_programs(program_uuids)
            )
    ...
    def course_run_detail(
        self, request, pk, course_id
    ):  # pylint: disable=unused-argument
        """
    ...
    @permission_required(
        "enterprise.can_view_catalog",
        fn=lambda request, pk, program_uuid: get_enterprise_customer_from_catalog_id(
            pk
        ),
    )
    def program_detail(
        self, request, pk, program_uuid
    ):  # pylint: disable=unused-argument
        """
...
class EnterpriseCustomerReportingConfigurationViewSet(EnterpriseReadWriteModelViewSet):
    ...
    @permission_required(
        ...
        fn=lambda request, *args, **kwargs: get_ent_cust_from_report_config_uuid(
            kwargs["uuid"]
        ),
    )
    def retrieve(self, request, *args, **kwargs):
    ...
    @permission_required(
        "enterprise.can_manage_reporting_config",
        fn=lambda request, *args, **kwargs: get_enterprise_customer_from_user_id(
            request.user.id
        ),
    )
    def list(self, request, *args, **kwargs):
...
class CouponCodesView(APIView):
    ...
    def get_required_query_params(self, request):
        ...
        enterprise_name = get_request_value(
            request, self.REQUIRED_PARAM_ENTERPRISE_NAME, ""
        )
        number_of_codes = get_request_value(
            request, self.OPTIONAL_PARAM_NUMBER_OF_CODES, ""
        )
    ...
    def post(self, request):
        ...
        try:
            (
                ...
                number_of_codes,
                notes,
            ) = self.get_required_query_params(request)
        except CodesAPIRequestError as invalid_request:
            return Response(
                {"error": str(invalid_request)}, status=HTTP_400_BAD_REQUEST
            )

        subject_line = _(
            "Code Management - Request for Codes by {token_enterprise_name}"
        ).format(token_enterprise_name=enterprise_name)
        ...
        try:
            ...
            LOGGER.info(
                "[Enterprise API] Coupon code request emails sent: %s", messages_sent
            )
...
class NotificationReadView(APIView):
    ...
    def get_required_query_params(self, request):
        ...
        """
        enterprise_slug = get_request_value(
            request, self.REQUIRED_PARAM_ENTERPRISE_SLUG, ""
        )
        notification_id = get_request_value(
            request, self.REQUIRED_PARAM_NOTIFICATION_ID, ""
        )
    ...
    def post(self, request):
        ...
        try:
            ...
            LOGGER.error(
                ...
                " User ID:{}, Exception:{}.".format(
                    notification_id, enterprise_slug, request.user.id, exc
                )
            )
            return Response(
...
class EnterpriseCustomerReportTypesView(APIView):
    ...
    def _get_data_types_with_recent_progress_type(data_types):
        ...
        """
        progress_data_types = [
            data_type for data_type in data_types if data_type[1].startswith("progress")
        ]
        progress_data_types.sort(key=lambda data_type: data_type[1])
        data_types_for_frontend = [
            data_type
            for data_type in data_types
            if not data_type[1].startswith("progress")
        ]
    ...
    def _get_data_types_for_non_pearson_customers(data_types):
        ...
        for data_type in data_types:
            if (
                data_type[1]
                not in models.EnterpriseCustomerReportingConfiguration.MANUAL_REPORTS
            ):
    ...
    def get(self, request, enterprise_uuid):
        ...
        if not enterprise_customer:
            return Response(
                {"detail": "Could not find the enterprise customer."},
        ...
        data_types_for_frontend = self._get_data_types_with_recent_progress_type(
            list(choices.get("data_type", []))
        )
...
class EnterpriseCustomerInviteKeyViewSet(EnterpriseReadWriteModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        invite_key = get_object_or_404(
            models.EnterpriseCustomerInviteKey, pk=kwargs["pk"]
        )
    ...
    def link_user(self, request, pk=None):
        ...
        enterprise_customer_key = get_object_or_404(
            models.EnterpriseCustomerInviteKey, uuid=pk
        )
        ...
        (
            enterprise_user,
            created,
        ) = models.EnterpriseCustomerUser.all_objects.get_or_create(
        ...
        if created:
            ...
            try:
                models.EnterpriseCustomerUser.all_objects.link_user(
                    enterprise_customer, request.user.email
                )
            except LinkUserToEnterpriseError:
