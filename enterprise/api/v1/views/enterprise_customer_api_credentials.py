...
class APICredentialsViewSet(EnterpriseReadWriteModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        user = request.user
        if not has_api_credentials_enabled(kwargs['enterprise_uuid']):
        ...
        if not queryset:
            return Response({'detail': f"No Api Application credentials found for user: {user.username}. "
                                       f"You must generate client credentials first."
                             },
    ...
    def regenerate_credentials(self, request, *args, **kwargs):
        """
        Method: PATCH

        URL: /enterprise/api/v1/enterprise_customer_api_credentials/regenerate_credentials/{enterprise_uuid}
        """
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        URL: /enterprise/api/v1/enterprise_customer_api_credentials/{enterprise_uuid}/{user_id}
        """
        if not has_api_credentials_enabled(kwargs['enterprise_uuid']):
            return Response({'detail': 'Can not access API credential viewset.'}, status=status.HTTP_403_FORBIDDEN)
