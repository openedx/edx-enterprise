...
class EnterpriseCustomerAdminViewSet(
    ...
    @action(
        detail=False,
        methods=['delete'],
        url_path=r'(?P<enterprise_customer_uuid>[A-Za-z0-9-]+)/admins/(?P<id>[^/.]+)'
    )
    def delete_admin(self, request, enterprise_customer_uuid=None, id=None):  # pylint: disable=redefined-builtin,unused-argument
        ...
        Based on the role query parameter:
                ...
                - If role='pending': Hard deletes PendingEnterpriseCustomerAdminUser where id=id
        - If role='admin': Deletes role assignment from SystemWideEnterpriseUserRoleAssignment
                    for the EnterpriseCustomerUser id=id, and deactivates
