...
from enterprise.models import EnterpriseCustomerCatalog, EnterpriseCatalogQuery
        ...
        if EnterpriseCatalogQuery.objects.filter(id=new_id).first():
            ...
            for catalog in catalogs_to_change:
                if not catalog.next_record and catalog.get_history_type_display() != 'Deleted':
                    uuids_to_change.append(str(catalog.uuid))

            EnterpriseCustomerCatalog.objects.filter(uuid__in=uuids_to_change).update(enterprise_catalog_query_id=new_id)
            LOGGER.info(
