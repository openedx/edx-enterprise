...
class ValidationMessages(object):
    ...
    BULK_LINK_FAILED = _("Error: Learners could not be added. Correct the following errors.")
    COURSE_AND_PROGRAM_ERROR = _("Either \"Course ID\" or \"Program ID\" can be specified, but both were.")
    COURSE_MODE_INVALID_FOR_COURSE = _("Enrollment track {course_mode} is not available for course {course_id}.")
    COURSE_MODE_NOT_AVAILABLE = _(
        "Enrollment track {mode} is not available for all courses in  {program_title}. The available enrollment tracks are {modes}."
    )
    COURSE_WITHOUT_COURSE_MODE = _("Select a course enrollment track for the  course or program.")
    ...
    INVALID_EMAIL_OR_USERNAME = _("{argument} does not appear to be a valid email address or known username.")
    INVALID_PROGRAM_ID = _(
        "Could not retrieve details for the program {program_id}. Specify a valid program ID or program name."
    )
    ...
    MULTIPLE_PROGRAM_MATCH = _(
        "Searching programs by title returned {program_count} programs. Try using program UUID."
    )
    ...
    USER_ALREADY_REGISTERED = _("A learner with the email address {email} is already registered with Enterprise Customer {ec_name}")
