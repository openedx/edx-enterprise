    ...
    def __init__(self, user):
        ...
        if CatalogIntegration is None:
            raise NotConnectedToOpenEdX(
                _('To get a CatalogIntegration object, this package must be installed in an Open edX environment.')
            )
        if get_edx_api_data is None:
            raise NotConnectedToOpenEdX(
                _('To parse a catalog API response, this package must be installed in an Open edX environment.')
            )
        if course_discovery_api_client is None:
            raise NotConnectedToOpenEdX(
                _('To get a catalog API client, this package must be installed in an Open edX environment.')
            )
