...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # The global operator user applies across all enterprises, so it is
        # seeded once and never linked to a specific enterprise.
        LOGGER.info('\nCreating global enterprise operator user...')
        seed_global_operator_user()
        ...
        lms_users = [
            get_or_create_enterprise_user(
                username=f'enterprise_learner_{slug}',
                role=ENTERPRISE_LEARNER_ROLE,
                enterprise_customer=enterprise_customer,
            ),
            get_or_create_enterprise_user(
                username=f'enterprise_admin_{slug}',
