...
class DegreedConfigurationViewSetTests(APITest):
    ...
    def test_soft_deleted_content_in_lists(self, mock_current_request):
        ...
        assert len(data) == 1
        assert data[0]['id'] == self.degreed_config.id
        assert len(DegreedEnterpriseCustomerConfiguration.all_objects.all()) == 2
        assert len(DegreedEnterpriseCustomerConfiguration.objects.all()) == 1
