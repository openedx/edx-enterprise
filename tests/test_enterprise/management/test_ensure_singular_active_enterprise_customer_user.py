...
class CreateEnterpriseCourseEnrollmentCommandTests(TestCase):
    ...
    def setUp(self):
        ...
        self.ecu_1 = EnterpriseCustomerUserFactory(user_id=self.lms_user_id, enterprise_customer=self.enterprise_customer_1, active=True)
        self.ecu_2 = EnterpriseCustomerUserFactory(user_id=self.lms_user_id, enterprise_customer=self.enterprise_customer_2, active=True)
        super().setUp()

    def test_bulk_query_update_only_changes_provided_query(self):
        """
        Test that the `ensure_singular_active_enterprise_customer_user` command will ensure only one ECU object is active for a given LMS user id.
        """
