...
def _mock_tpa_classes():
    """
    Build mock TPA classes: (SAMLProviderConfig, SAMLProviderConfigSerializer).
    """
    MockSAMLProviderConfig = MagicMock()
    MockSerializer = MagicMock()
    return MockSAMLProviderConfig, MockSerializer
...
class TestSAMLProviderConfigViewSet(APITest):
    ...
    @patch('enterprise.api.v1.views.saml_provider_config.SAMLProviderConfigViewSet._get_tpa_classes')
    def test_get_queryset_filters_by_enterprise_idp(self, mock_tpa):
        MockSAMLProviderConfig, MockSerializer = _mock_tpa_classes()
        ...
        mock_tpa.return_value = (MockSAMLProviderConfig, MockSerializer)
        ...
        MockSAMLProviderConfig.objects.current_set.return_value = [mock_config]
        MockSAMLProviderConfig.objects.filter.return_value = MockQuerySet([mock_config])

        url = f'{PROVIDER_CONFIG_LIST_URL}?enterprise-id={self.enterprise_uuid}'
        self.client.get(settings.TEST_SERVER + url)

        MockSAMLProviderConfig.objects.filter.assert_called_once_with(id__in=[1])
    ...
    @patch('enterprise.api.v1.views.saml_provider_config.SAMLProviderConfigViewSet._get_tpa_classes')
    def test_create_saves_config_and_creates_idp_association(self, mock_tpa):
        MockSAMLProviderConfig, MockSerializer = _mock_tpa_classes()
        mock_tpa.return_value = (MockSAMLProviderConfig, MockSerializer)
    ...
    @patch('enterprise.api.v1.views.saml_provider_config.SAMLProviderConfigViewSet._get_tpa_classes')
    def test_create_raises_validation_error_on_integrity_error(self, mock_tpa):
    ...
    @patch('enterprise.api.v1.views.saml_provider_config.SAMLProviderConfigViewSet._get_tpa_classes')
    def test_destroy_archives_config_and_deletes_idp(self, mock_tpa, mock_get_object):
        MockSAMLProviderConfig, MockSerializer = _mock_tpa_classes()
        mock_tpa.return_value = (MockSAMLProviderConfig, MockSerializer)
        ...
        MockSAMLProviderConfig.objects.filter.assert_called_once_with(id=42)
        MockSAMLProviderConfig.objects.filter().update.assert_called_once_with(archived=True, enabled=False)
    ...
    @patch('enterprise.api.v1.views.saml_provider_config.SAMLProviderConfigViewSet.get_object')
    @patch('enterprise.api.v1.views.saml_provider_config.SAMLProviderConfigViewSet._get_tpa_classes')
    def test_destroy_raises_validation_error_when_customer_not_found(self, mock_tpa, mock_get_object):
        MockSAMLProviderConfig, MockSerializer = _mock_tpa_classes()
        mock_tpa.return_value = (MockSAMLProviderConfig, MockSerializer)
