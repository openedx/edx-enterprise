...
@mark.django_db
@mock.patch('enterprise.api_client.lms.JwtBuilder', mock.Mock())
def test_get_content_metadata_with_catalogs_last_modified():
    client = enterprise_catalog.EnterpriseCatalogApiClient('staff-user-goes-here')
    page_size = client.GET_CONTENT_METADATA_PAGE_SIZE
    catalog = EnterpriseCustomerCatalogFactory()
    ...
    catalogs_last_modified = {
        str(catalog.uuid): test_modified_datetime
    }
    expected_response = {
        'count': 100,
        'next': None,
    ...
    responses.add(responses.GET, url, json=expected_response)
    results, content_catalog_last_modified = client.get_content_metadata(
        enterprise_catalogs=[catalog_data],
    ...
    expected_last_modified = {
        ('key-{}'.format(index)): test_modified_datetime for index in range(page_size)
    }
    assert expected_last_modified == content_catalog_last_modified
