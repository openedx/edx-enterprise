...
upgrade: check_pins  ## update the requirements/*.txt files with the latest packages satisfying requirements/*.in
	...
	$(PIP_COMPILE) --no-emit-trusted-host --no-emit-index-url --allow-unsafe -o requirements/pip.txt requirements/pip.in
	$(PIP_COMPILE) --no-emit-trusted-host --no-emit-index-url -o requirements/pip-tools.txt requirements/pip-tools.in
