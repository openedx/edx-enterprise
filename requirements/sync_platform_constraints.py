...
def build_constraint_array(existing, added):
    """Render constraint-dependencies with the platform-synced pins visibly fenced off.

    Once appended as plain strings, a platform pin is indistinguishable from an
    edx_lint-managed one -- a reviewer scanning the 300+ line array has no way to
    tell them apart. Wrapping the platform block in comment markers makes that
    provenance visible directly in pyproject.toml, not just in the header comment
    above the array.
    """
    lines = ["constraint-dependencies = ["]
    lines += [f'    "{item}",' for item in existing]
    if added:
        lines.append("    # --- begin openedx-platform sync (auto-generated by")
        lines.append("    # requirements/sync_platform_constraints.py -- do not hand-edit,")
        lines.append("    # re-run `make upgrade` to refresh) ---")
        lines += [f'    "{item}",' for item in added]
        lines.append("    # --- end openedx-platform sync ---")
    lines.append("]")
    return tomlkit.parse("\n".join(lines))["constraint-dependencies"]
...
def main():
    ...
    doc["tool"]["uv"]["constraint-dependencies"] = build_constraint_array(existing, added)
